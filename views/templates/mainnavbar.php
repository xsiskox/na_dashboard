<nav class="navbar navbar-default navbar-static-top " role="navigation" style="margin-bottom: 0">
	<div class="navbar-header">
		...
		<?php $users=wp_get_current_user();?>
	<a class="navbar-brand" href="/index.php"><?php echo $users->last_name." ".$users->ID;?> <span class="glyphicon glyphicon-home" aria-hidden="true"></span></a>
	</div>
	<!-- /.navbar-header -->
			...
			<ul class="nav" id="side-menu">
				...
				foreach($data as $item)
				{
					if($item['ddm']==0)
					{
						if($item['parent']=='')
						{
							echo'<li> <a role="button" name="menuButton" id="'.$item['title'].'"><i class="fa fa-fw '.$item['icon'].'"></i>'.$item['title'].' </a> </li>';
						}
					}
					else
					{
						$filter=$item['title'];
						$filterArray=array_filter($data,function($var)use($filter)
							{
								return $var['parent']==$filter;;													
							});
						
						echo'<li> <a role="button" name="" id="'.$item['title'].'"><i class="fa fa-fw '.$item['icon'].'"></i>'.$item['title'].' <span class="fa arrow"></a>';
						echo'<ul class="nav nav-second-level">';
						foreach($filterArray as $sub_item)
						{
							echo'<li> <a role="button" name="menuButton" id="'.$sub_item['title'].'"><i class="fa fa-fw '.$sub_item['icon'].'"></i>'.$sub_item['title'].'</a> </li>';
						}
						echo'</li></ul>';
					}
				}
